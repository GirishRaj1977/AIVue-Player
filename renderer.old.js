// Global Error & Log Catching
const origConsoleErr = console.error;
const origConsoleLog = console.log;
console.error = (...args) => {
    origConsoleErr(...args);
    if (window.iptvAPI && window.iptvAPI.logToMain) {
        window.iptvAPI.logToMain('ERROR', ...args);
    }
};
console.log = (...args) => {
    origConsoleLog(...args);
    if (window.iptvAPI && window.iptvAPI.logToMain) {
        window.iptvAPI.logToMain('INFO', ...args);
    }
};
window.addEventListener('unhandledrejection', (event) => {
    if (window.iptvAPI && window.iptvAPI.logToMain) {
        window.iptvAPI.logToMain('ERROR', 'Unhandled Rejection:', event.reason ? event.reason.stack || event.reason : event);
    }
});
window.addEventListener('error', (event) => {
    if (window.iptvAPI && window.iptvAPI.logToMain) {
        window.iptvAPI.logToMain('ERROR', 'Uncaught Error:', event.error ? event.error.stack || event.error : event.message);
    }
});
...
let channelList, playerContainer, clearBtn, loadingMsg;
let importNameInput, btnModeFile, btnModeUrl, containerFile, containerUrl;
let importFilePath, importUrlPath, importBrowseBtn, importSubmitBtn, importEpgInput, importCancelBtn;
...
async function autoMapChannels(showSummaryAlert = false) {
    ...
    for (const ch of allChannels) {
        ...
        if (matchedEpgId) {
            ...
            newMappings[title] = matchedEpgId;
            mappedCount++;
        }
    }

    if (mappedCount > 0) {
        await window.iptvAPI.saveMappingsBulk(newMappings);
        updateState();
...
function updateNavLockState() {
    ...
    if (btnLiveTv) {
        ...
        btnLiveTv.disabled = !hasChannels;
        btnLiveTv.style.opacity = hasChannels ? '1' : '0.3';
        btnLiveTv.style.cursor = hasChannels ? 'pointer' : 'not-allowed';
    }
    if (btnEpg) {
        btnEpg.disabled = !hasPlaylists;
        btnEpg.style.opacity = hasPlaylists ? '1' : '0.3';
        btnEpg.style.cursor = hasPlaylists ? 'pointer' : 'not-allowed';
        btnEpg.disabled = !hasChannels;
        btnEpg.style.opacity = hasChannels ? '1' : '0.3';
        btnEpg.style.cursor = hasChannels ? 'pointer' : 'not-allowed';
    }
    
    // Ensure Settings is ALWAYS active so the user can manage the app
    if (btnSettings) {
    ...
    // Keep "Coming Soon" buttons perpetually locked for now
    ...
    try {
        ...
        if (result && result.error) {
            ...
            if (allEpgSources.length > 0) {
                const combinedEpgs = allEpgSources.join(',');
                window.iptvAPI.updateEpg(combinedEpgs, null, true)
                    .then(async () => {
                        epgChannelsData = await window.iptvAPI.getEpgChannels(combinedEpgs);
...
async function fetchEpgDataForChannels(channels) {
    ...
    const epgIdsArr = Array.from(epgIdsToFetch).filter(id => !epgCache[id]);
    if (epgIdsArr.length === 0) return;
    
    const { gridStart, gridEnd, totalWidth, pxPerMinute, now } = epgGridState;
    const fetchStart = new Date(gridStart.getTime() - 24 * 60 * 60 * 1000);
    const fetchEnd = new Date(gridEnd.getTime() + 24 * 60 * 60 * 1000);
...
function renderVisibleEpgRows(force = false) {
    ...
    if (!force && startIndex === epgLastStartIndex && endIndex === epgLastEndIndex) {
        return;
    }
    ...
    for (let i = startIndex; i <= endIndex; i++) {
        ...
        let epgId = mappedId || channel.tvg_id || channel.tvg_name;
        ...
        } else if (!programmes && epgId) {
            html += `<div class="epg-play-channel" data-index="${globalIdx}" style="display: flex; align-items: center; padding-left: 20px; height: 100%; color: #888; font-size: 0.9em; width: 100%; cursor: pointer;">Loading...</div>`;
        } else {
    ...
    if (missingChannels.length > 0) {
        fetchEpgDataForChannels(missingChannels);
    }
}
...
async function renderFullEpg() {
    ...
    for (let i = 0; i < 24; i++) {
        ...
        headerHtml += `<div style="position: absolute; left: ${i * hourWidth}px; width: ${hourWidth}px; height: 100%; border-right: 1px solid #333; display: flex; align-items: center; padding-left: 10px; color: #888; box-sizing: border-box;">${timeStr}</div>`;
    }
    ...
    if (nowPx > 0 && nowPx < totalWidth) {
        redLineHtml = `<div style="position: absolute; left: ${nowPx}px; top: 0; height: 100%; width: 2px; background: #cf6679; z-index: 15; pointer-events: none;"></div>`;
    }
...
if (btnExitEl) {
    btnExitEl.addEventListener('click', () => { window.close(); });
}
...
function switchTab(tabId, clickedBtn) {
    ...
    // Dynamically reduce the channel list (sidebar) width by 30% upon first showing it
    if (isLive && sidebar && !window.sidebarReduced) {
        ...
        if (!isNaN(currentWidth) && currentWidth > 0) {
            const newWidth = currentWidth * 0.7; // Reduce by 30%
...
const btnLiveTvEl = document.getElementById('btn-live-tv');
if (btnLiveTvEl) btnLiveTvEl.addEventListener('click', function() { if (!this.disabled) switchTab('live-tv', this); });
const btnPlaylistEl = document.getElementById('btn-playlist');
if (btnPlaylistEl) btnPlaylistEl.addEventListener('click', function() { switchTab('playlist', this); });
const btnEpgEl = document.getElementById('btn-epg');
if (btnEpgEl) btnEpgEl.addEventListener('click', function() { if (!this.disabled) switchTab('epg', this); });
const btnSettingsEl = document.getElementById('btn-settings');
if (btnSettingsEl) btnSettingsEl.addEventListener('click', function() { if (!this.disabled) switchTab('settings', this); });
...
// Initialization wrapper to prevent DOMContentLoaded race conditions
async function initializeApp() {
    ...
    importNameInput = document.getElementById('import-name');
    btnModeFile = document.getElementById('btn-mode-file');
    btnModeUrl = document.getElementById('btn-mode-url');
    containerFile = document.getElementById('input-container-file');
    containerUrl = document.getElementById('input-container-url');
    importFilePath = document.getElementById('import-file-path');
    importUrlPath = document.getElementById('import-url-path');
    importBrowseBtn = document.getElementById('import-browse-btn');
    importSubmitBtn = document.getElementById('import-submit-btn');
    importEpgInput = document.getElementById('import-epg-path');
    importCancelBtn = document.getElementById('import-cancel-btn');
    
    // --- Explicitly bind standard event listeners exactly to their inputs ---
    if (btnModeFile) {
    ...
    if (importBrowseBtn) {
        importBrowseBtn.addEventListener('click', async (e) => {
            ...
            if (filePaths && filePaths.length > 0) {
                if (importFilePath) importFilePath.value = filePaths[0];
    ...
    if (importSubmitBtn) {
        importSubmitBtn.addEventListener('click', async (e) => {
            e.preventDefault();
            const name = importNameInput ? importNameInput.value.trim() : '';
            if (!name) return alert("Playlist name is mandatory.");
            let source = '';
            if (currentImportMode === 'file') {
                source = importFilePath ? importFilePath.value.trim() : '';
                if (!source) return alert("Please select a file location.");
            } else {
                source = importUrlPath ? importUrlPath.value.trim() : '';
    ...
    if (importUrlPath) {
        importUrlPath.addEventListener('change', async (e) => {
    ...
    const btnExitEl = document.getElementById('btn-exit');
    if (btnExitEl) btnExitEl.addEventListener('click', () => { window.close(); });
    ...
    if (navBarNode) {
        ...
        ['Movies', 'VOD', 'Recording'].forEach(name => {
            ...
            btn.style.justifyContent = 'center';
            btn.innerHTML = comingSoonIcons[name] || `<span style="font-weight: bold;">${name}</span>`;
            btn.addEventListener('click', function() { if (!this.disabled) alert('Coming in Next Version'); });
            navBarNode.appendChild(btn);
        });
        const btnOrder = ['btn-live-tv', 'btn-playlist', 'btn-epg', 'btn-movies', 'btn-vod', 'btn-recording', 'btn-settings', 'btn-exit'];
        btnOrder.forEach(id => { const btn = document.getElementById(id); if (btn) navBarNode.appendChild(btn); });
        Array.from(navBarNode.children).forEach(child => { if (child.tagName === 'DIV' && child.children.length === 0) child.remove(); });
    }

    // Force text inputs and buttons to be active (in case the HTML had them disabled by default or form state locked)
    ['import-name', 'import-file-path', 'import-url-path', 'import-epg-path',
     'btn-mode-file', 'btn-mode-url', 'import-browse-btn', 'import-submit-btn', 'import-cancel-btn'
    ].forEach(id => {
